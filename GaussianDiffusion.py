        ...
        if random_param:
            ...
            noise[:, i, ...] = torch.unsqueeze(
                    torch.from_numpy(
                            ...
                            #Simplex_instance.rand_2d_fixed_T_octaves(
...
class GaussianDiffusionModel:
    ...
    def forward_backward(
            self, model, x, see_whole_sequence="whole", t_distance=None, denoise_fn="gauss",
            ):
        ...
        if see_whole_sequence == "whole":
            ...
            seq.append(self.noise_fn(x, t_tensor).float().cpu().detach())
            x = self.sample_q(
                    x, t_tensor,
    ...
    def p_loss(self, model, x_0, config):
        if self.loss_weight == "none":
            if config.training.train_start:
                t = torch.randint(
                        0, min(config.training.sample_distance, self.num_timesteps), (x_0.shape[0],),
                        device=x_0.device
                        )
            else:
